...
def size_display():
    ...
    global distance, grey_pill, grey_star, timer_pill, timer_star

    distance += scrool_all/30 * dt
    ...
    timer_surf = font_dist.render(f'TIMER - {(pygame.time.get_ticks() / 1000):.2f}', True, (240, 240, 240))
    timer_rect = timer_surf.get_rect(center=(WINDOW_WIDTH - 160, 240))
    display_surface.blit(timer_surf, timer_rect)

    dist_surf = font_dist.render(f'DISTANCE - {distance:.2f}', True, (240, 240, 240))
    ...
    special_rect = special_surf.get_rect(center=(WINDOW_WIDTH - 160, 590))
...
running = init(display_surface, clock, WINDOW_WIDTH, WINDOW_HEIGHT)
...
while running:
    ...
    qnt_moedas, index, distance, nitro_capacity, nitro_blocked = 0, 0, 0, 0, 0
    pill_blocked, star_bloked, grey_pill, grey_star = 0, 0, 0, 0
    ...
    if exit == False:
        running = end(font_base, display_surface, WINDOW_WIDTH, WINDOW_HEIGHT)
    else:
        running = False
